    ...
    container.innerHTML = sorted.map(([acc, bal]) => `
        <div class="glass bg-white/40 dark:bg-dark-surface/40 p-3 md:p-4 rounded-2xl md:rounded-3xl border border-brand-200/30 dark:border-dark-border/50 flex items-center gap-2 md:gap-3 hover:shadow-md transition-shadow">
            ...
            <div class="min-w-0 flex-1">
                <p class="text-[9px] md:text-xs font-bold text-brand-300 uppercase truncate">${acc}</p>
                <p class="text-sm md:text-base font-black break-words ${bal >= 0 ? 'text-emerald-600' : 'text-rose-500'}">${formatIDR(bal)}</p>
            </div>
        </div>
    `).join('');
...
function renderBudgetSettings() {
    ...
    container.innerHTML = CATEGORIES.expense.map(cat => `
        <div class="flex items-center gap-2 md:gap-3 bg-white/50 dark:bg-dark-surface/50 p-3 md:p-4 rounded-2xl md:rounded-3xl">
            <div class="w-7 h-7 md:w-9 md:h-9 rounded-xl md:rounded-2xl bg-white dark:bg-dark-surface flex items-center justify-center text-brand-400 shadow-sm text-xs md:text-base"><i class="fa-solid ${CATEGORY_ICONS[cat]}"></i></div>
...
function renderFullTransactions() {
    ...
    document.getElementById('full-tx-body').innerHTML = filtered.map(t => `
        <tr class="hover:bg-brand-100/50 dark:hover:bg-dark-border/30 transition">
            <td class="px-4 md:px-6 py-3 md:py-4"><div class="flex items-center gap-2 md:gap-3"><div class="w-7 h-7 md:w-9 md:h-9 rounded-xl md:rounded-2xl bg-white/60 dark:bg-dark-surface flex items-center justify-center"><i class="fa-solid ${CATEGORY_ICONS[t.category]||'fa-tag'} text-brand-500 text-xs md:text-base"></i></div><div><p class="font-bold text-xs md:text-sm text-[#5a4e4e] dark:text-brand-300">${t.desc||'—'}</p><p class="text-[8px] md:text-[9px] text-brand-400 uppercase">${t.date}</p></div></div></td>
            <td class="px-4 md:px-6 py-3 md:py-4"><span class="px-1.5 md:px-2 py-0.5 md:py-1 rounded-xl text-[8px] md:text-[9px] font-black uppercase bg-brand-200/50 dark:bg-dark-surface text-brand-600 dark:text-brand-300">${t.category}</span></td>
            <td class="px-4 md:px-6 py-3 md:py-4"><span class="text-[10px] md:text-xs font-semibold"><i class="fa-regular fa-building mr-1 text-brand-400"></i>${t.account}</span></td>
            <td class="px-4 md:px-6 py-3 md:py-4 text-right font-black text-xs md:text-sm whitespace-nowrap ${t.type==='income'?'text-emerald-500':'text-[#5a4e4e] dark:text-white'}">${t.type==='income'?'+':'-'} ${formatIDR(t.amount)}</td>
            <td class="px-4 md:px-6 py-3 md:py-4 text-center"><button onclick="deleteTx(${t.id})" class="w-6 h-6 md:w-7 md:h-7 rounded-full text-brand-400 hover:text-rose-500 hover:bg-rose-50/50 dark:hover:bg-rose-500/10 transition-all"><i class="fa-solid fa-trash-can text-[10px] md:text-xs"></i></button></td>
        </tr>`).join('');
