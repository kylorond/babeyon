...
function renderAccounts() {
    ...
    container.innerHTML = accounts.map(acc => {
        ...
        return `
        <div class="glass bg-white/40 dark:bg-dark-surface/40 p-4 rounded-2xl border border-brand-200/30 dark:border-dark-border/50 flex items-center justify-between hover:shadow-md transition-shadow">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-brand-100/60 dark:bg-brand-500/20 flex items-center justify-center text-brand-500 dark:text-brand-400">
                ...
                <div>
                    ...
                    <p class="text-[9px] text-brand-400">${acc.currency} • Saldo awal ${formatIDR(acc.initialBalance, acc.currency)}</p>
                </div>
            </div>
            <div class="text-right">
                <p class="font-black text-sm ${balance >= 0 ? 'text-emerald-600' : 'text-rose-500'}">${formatIDR(balance, acc.currency)}</p>
                <div class="flex gap-1 mt-1">
                    <button onclick="editAccount(${acc.id})" class="text-xs text-brand-400 hover:text-brand-500"><i class="fa-regular fa-pen-to-square"></i></button>
                    <button onclick="deleteAccount(${acc.id})" class="text-xs text-brand-400 hover:text-rose-500"><i class="fa-regular fa-trash-can"></i></button>
                </div>
            </div>
        </div>`;
    }).join('');
...
function renderCategories() {
    ...
    incomeList.innerHTML = (categories.income || []).map(cat => `
        <div class="flex items-center justify-between p-2 bg-white/40 dark:bg-dark-surface/40 rounded-xl">
            ...
            <button onclick="deleteCategory('income', '${cat}')" class="text-brand-400 hover:text-rose-500"><i class="fa-regular fa-trash-can text-xs"></i></button>
        </div>
    `).join('');
    expenseList.innerHTML = (categories.expense || []).map(cat => `
        <div class="flex items-center justify-between p-2 bg-white/40 dark:bg-dark-surface/40 rounded-xl">
            <span class="text-sm">${cat}</span>
            <button onclick="deleteCategory('expense', '${cat}')" class="text-brand-400 hover:text-rose-500"><i class="fa-regular fa-trash-can text-xs"></i></button>
        </div>
    `).join('');
}
...
function renderAccountBalances() {
    ...
    container.innerHTML = sorted.map(([accId, bal]) => {
        ...
        if (!acc) return '';
        return `
        <div class="glass bg-white/40 dark:bg-dark-surface/40 p-3 md:p-4 rounded-2xl md:rounded-3xl border border-brand-200/30 dark:border-dark-border/50 flex items-center gap-2 md:gap-3 hover:shadow-md transition-shadow">
            <div class="w-8 h-8 md:w-10 md:h-10 rounded-xl md:rounded-2xl bg-brand-100/60 dark:bg-brand-500/20 flex items-center justify-center text-brand-500 dark:text-brand-400 text-sm md:text-base">
                <i class="fa-solid ${acc.name.toLowerCase().includes('cash') ? 'fa-money-bill' : 'fa-building-columns'}"></i>
            </div>
            <div class="min-w-0 flex-1">
                <p class="text-[9px] md:text-xs font-bold text-brand-300 uppercase truncate">${acc.name}</p>
                <p class="text-sm md:text-base font-black break-words ${bal >= 0 ? 'text-emerald-600' : 'text-rose-500'}">${formatIDR(bal, acc.currency)}</p>
            </div>
        </div>`;
    }).join('');
...
function renderDashboardList(data) {
    ...
    list.innerHTML = data.slice(0, 5).map(t => `<tr class="group hover:bg-brand-100/50 dark:hover:bg-dark-border/30 transition">
        <td class="px-4 md:px-6 py-3 md:py-4"><div class="flex items-center gap-2 md:gap-3"><div class="w-7 h-7 md:w-9 md:h-9 rounded-xl md:rounded-2xl bg-brand-200/50 dark:bg-dark-surface flex items-center justify-center text-brand-500 text-xs md:text-base"><i class="fa-solid ${CATEGORY_ICONS[t.category] || 'fa-tag'}"></i></div><div><p class="font-bold text-xs md:text-sm text-[#5a4e4e] dark:text-brand-300">${t.desc || t.category}</p><p class="text-[8px] md:text-[9px] text-brand-400 font-bold uppercase">${t.date}</p></div></div></td>
...
function renderBudgets(data) {
    ...
    (categories.expense || []).forEach(cat => {
        ...
        if (limit > 0) {
            ...
            totalLimit += limit;
            totalSpent += spent;
            html += `<div><div class="flex justify-between text-[10px] md:text-xs font-bold mb-1"><span>${cat}</span><span class="${perc >= 90 ? 'text-rose-500' : 'text-brand-400'}">${perc.toFixed(0)}%</span></div>
            <div class="h-1.5 md:h-2 w-full bg-brand-200/60 dark:bg-dark-border rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-brand-400 to-brand-500 transition-all duration-700" style="width:${perc}%"></div></div>
            <div class="flex justify-between text-[8px] md:text-[9px] font-bold text-brand-400 mt-1"><span>${formatIDR(spent)}</span><span>${formatIDR(limit)}</span></div></div>`;
        }
    });
    ...
    donutChart = new Chart(donutCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#ffb6c1','#f9c5d1','#ff9eb5','#f28b9c','#e5738a','#d45e78','#b84a62','#9e3b52'],
                borderWidth: 0,
                hoverOffset: 8
            }]
        },
        options: { cutout: '75%', plugins: { legend: { display: false }, tooltip: { backgroundColor: '#3a2f2f', titleColor: '#fff5f5' } } }
    });
...
function renderDebts() {
    ...
    container.innerHTML = debts.map(d => {
        ...
        const statusClass = d.status === 'paid' ? 'text-emerald-600' : isOverdue ? 'text-rose-500' : 'text-amber-500';
        ...
        <div class="glass bg-white/40 dark:bg-dark-surface/40 p-4 rounded-2xl border border-brand-200/30 dark:border-dark-border/50 hover:shadow-md transition-shadow">
            <div class="flex justify-between items-start">
                <div>
                    <h4 class="font-bold text-sm md:text-base text-brand-700 dark:text-brand-300">${d.name}</h4>
                    <p class="text-[10px] md:text-xs text-brand-400 mt-1">${d.date} ${d.dueDate ? '· Jatuh tempo ' + d.dueDate : ''}</p>
                </div>
                <div class="flex gap-1">
                    <button onclick="toggleDebtStatus(${d.id})" class="text-brand-400 hover:text-brand-500"><i class="fa-regular fa-circle-check"></i></button>
                    <button onclick="deleteDebt(${d.id})" class="text-brand-400 hover:text-rose-500"><i class="fa-solid fa-trash-can text-xs"></i></button>
                </div>
            </div>
            <p class="text-xs md:text-sm font-bold mt-2 ${d.type === 'lend' ? 'text-emerald-600' : 'text-rose-500'}">${d.type === 'lend' ? 'Piutang' : 'Utang'} ${formatIDR(d.amount)}</p>
            <p class="text-[9px] md:text-xs text-brand-400 mt-1">${d.desc || ''} · <span class="${statusClass}">${statusText}</span></p>
        </div>`;
    }).join('');
...
function renderReminders() {
    ...
    container.innerHTML = reminders.map(r => {
        const overdue = r.date < today ? 'text-rose-500' : '';
        return `
        <div class="glass bg-white/40 dark:bg-dark-surface/40 p-4 rounded-2xl border border-brand-200/30 dark:border-dark-border/50 hover:shadow-md transition-shadow">
            <div class="flex justify-between items-start">
                <div>
                    <h4 class="font-bold text-sm md:text-base text-brand-700 dark:text-brand-300">${r.title}</h4>
                    <p class="text-[10px] md:text-xs text-brand-400 mt-1">${r.date} ${overdue ? '(Terlewat)' : ''}</p>
                </div>
                <button onclick="deleteReminder(${r.id})" class="text-brand-400 hover:text-rose-500"><i class="fa-solid fa-trash-can text-xs"></i></button>
            </div>
            ${r.amount ? `<p class="text-xs md:text-sm font-bold mt-2">${formatIDR(r.amount)}</p>` : ''}
            ${r.desc ? `<p class="text-[9px] md:text-xs text-brand-400 mt-1">${r.desc}</p>` : ''}
        </div>`;
    }).join('');
...
function renderGoals() {
    ...
    container.innerHTML = goals.map(g => {
        ...
        <div class="glass bg-white/40 dark:bg-dark-surface/40 p-4 rounded-2xl border border-brand-200/30 dark:border-dark-border/50 hover:shadow-md transition-shadow">
            ...
            <div class="mt-2">
                <div class="flex justify-between text-[9px] md:text-xs font-bold text-brand-400">
                    <span>${formatIDR(g.current)}</span>
                    <span>${formatIDR(g.target)}</span>
                </div>
                <div class="h-1.5 w-full bg-brand-200/60 dark:bg-dark-border rounded-full overflow-hidden mt-1">
                    <div class="h-full bg-gradient-to-r from-brand-400 to-brand-500 transition-all duration-700" style="width:${progress}%"></div>
                </div>
                <p class="text-[8px] md:text-[9px] text-brand-400 mt-2">Target: ${deadline}</p>
            </div>
        </div>`;
    }).join('');
...
function switchTab(tabId) {
    ...
    tabs.forEach(t => {
        ...
        if (btn) btn.className = "nav-item w-full flex items-center px-4 py-3 rounded-2xl text-brand-500 dark:text-brand-300 hover:bg-brand-100 dark:hover:bg-dark-border/50 font-medium transition-all gap-4";
    });
...
function renderBudgetSettings() {
    ...
    container.innerHTML = (categories.expense || []).map(cat => `
        <div class="flex items-center gap-2 md:gap-3 bg-white/50 dark:bg-dark-surface/50 p-3 md:p-4 rounded-2xl md:rounded-3xl">
            <div class="w-7 h-7 md:w-9 md:h-9 rounded-xl md:rounded-2xl bg-white dark:bg-dark-surface flex items-center justify-center text-brand-400 shadow-sm text-xs md:text-base"><i class="fa-solid ${CATEGORY_ICONS[cat] || 'fa-tag'}"></i></div>
